...
\textcolor{red}{On the other hand, Figure~\ref{fig:false} shows a background event due to a nucleus in the quenching gas (C or O).
The incident ${}^{10}\rm{C}$ beam was scattered at the large angle off a heavy nucleus from the beam axis in the background event.
If the incident ${}^{10}\rm{C}$ had been scattered with ${}^{4}\rm{He}$, it would not have scattered at a large angle.}
...
The previous method has problems of requiring the complex algorithm and a large computing power.
...
\textcolor{red}{The maxpooling layers reduce the size of feature maps by taking the maximum value.
  ...
  See Ref.~\cite{conv,dropout} for details.
}
...
\textcolor{red}{This network was trained and evaluated with the images from 3,000 events of the ${}^{10}{\rm C}+\alpha$
  events and background events tagged by eye scan,
