...
\abst{
  ...
  \textcolor{red}{These images are often analyzed} with the Hough transformation.
  This previous method requires \textcolor{red}{a} complex algorithm and a large computing power.
...
%ことで飛跡を検出する。
The anode and cathode electrodes of the $\mu$-PIC are segmented into 256 strips\textcolor{red}{, respectively,}
...
\textcolor{red}{On the other hand, Figure~\ref{fig:false} shows a background event due to a heavy nucleus in the quenching gas (C or O).
...
The energy of the recoil $\alpha$ particle is determined from the length of the track in \textcolor{red}{the detection gas consisting of the target gas and the quenching} gas,
...
\textcolor{red}{The anode and cathode images were analyzed with the following two steps;
...
%\end{itemize}
%の大きくわけて2つの解析が必要になる。
However, the \textcolor{red}{previous method} required a lot of efforts to
...
\section{\textcolor{red}{Previous method using the Hough transformation}}
...
\section{New method \textcolor{red}{using neural networks}}
...
\textcolor{red}{The maxpooling layers reduce the size of feature maps by taking the maximum value.
...
\textcolor{red}{This network was trained and evaluated with the images from 3,000 events of the ${}^{10}{\rm C}+\alpha$
...
\textcolor{red}{This network was trained and evaluated with the images from 4,566 events of the ${}^{10}\rm{C}+\alpha$ events.
...
\textcolor{red}{until the accuracy of the neural network fully converged}.
