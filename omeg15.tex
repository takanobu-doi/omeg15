...
\abst{
  In the scattering experiments with the MAIKo TPC, we can project tracks of charged particles
  onto two planes perpendicular and parallel to the beam axis and acquire these projections as two images.
  It is, therefore, necessary to analyze these two-dimensional images to reconstruct the original three-dimensional tracks.
  Conventionally, we analyze them with the Hough transformation which is a method
  to find lines in images.
  This conventional analysis method requires complex algorithms and large computing power.
  Recently neural networks have been widely employed for the image recognition.
  In the present work, we developed a new analysis method for the MAIKo TPC using neural networks.
...
In this experiment, $\rm{He}$ (96\%) was used as the target gas and $\rm{CO}_{2}$ (4\%) was used as the quench gas.
...
The tracks of charged particles in each event were projected onto the plane that is perpendicular to the anode strips and
the plane that is perpendicular to the cathode strips, and were recorded as the anode and cathode images.
The MAIKo TPC outputs two images in each event.
Figures \ref{fig:true} and \ref{fig:false} are examples of the acquired images.
These black-and-white images with $1024\times 256$ pixels present drift times
on the 256 strips of the anode and cathode recorded
...
recoiled at large angle.
On the other hand, Fig.~\ref{fig:false} is from a background event due to the quenching gas
...
%\end{itemize}
%の大きくわけて2つの解析が必要になる。
However, the conventional analysis method requires a lot of efforts to select events and extract track information from the anode and cathode images.
...
select the ${}^{10}\rm{C}+\alpha$ events and to extract tracks.
...
The conventional method has problems of requiring complex algorithms and large computing power.
...
Once a neural network trained, the neural network is able to recognize images faster and more accurately than the conventional method.
...
the input layer, the maxpooling layer, the convolutional layer, the full connection layer,
...
In the event selection, we obtain a probability that this pair of images were taken
in the ${}^{10}\rm{C}+\alpha$ event from output layer.
...
It took about 26 minutes for the training and about 1 second to process for 300 events.
...
It took about 270 minutes for the training and about 2 seconds to process 1,554 events.
The track endpoints determined by the neural network are compared with those
by the conventional method in Fig.~\ref{fig:result_detection} in a typical event.
...
and the neural network are about 4 mm in the standard deviation for the 1,554 events.
