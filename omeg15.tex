...
%\end{itemize}
%の大きくわけて2つの解析が必要になる。
However, the \textcolor{red}{method using Hough transformation} required a lot of efforts to
...
\textcolor{red}{
  The network in fig.~\ref{fig:selection} extracts the features from the inputted images in two filters in each branch,
  extracts the features from the merged feature map in one step, and judges with the feature map.
  The network in fig.~\ref{fig:extraction} extracts the features from the inputted images in three filters in each branch,
  and judges with the two feature maps.
}
%\textcolor{red}{Figure~\ref{fig:selection}ではconv. と poolのペアで特徴量の抽出を行い，2つの画像を合わせてさらなる抽出を行い，それらの特徴量を用いて判断を行う．
%Figure~\ref{fig:extraction}では，それぞれで特徴量の抽出を行い，それらの特徴量から最終的な判断を下す．}
