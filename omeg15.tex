...
cathode strips are parallel to $z$-axis as shown in Fig.~\ref{fig:MIAKo}. %は z軸の方向と平行である。
...
$\rm{He}$ or $\rm{H}_{2}$ gas is widely used for a target gas
but operation of TPC with pure $\rm{H}$ or $\rm{He}$ gas is unstable and prone to discharge.
...
Conventionally, the Hough transformation was used in order to remove background events.
...
The conventinal analysis has problems of requiring complex algorithms and large computing power.
...
Once a neural network trained, the neural network is able to recognize images more accurately and faster than the conventional analysis.
...
We inputted a pair of images from the MAIKo TPC to the neural network for the event selection and
obtain a probability that this pair of images were taken in the ${}^{10}\rm{C}+\alpha$ event.
...
The neural network for the track extraction outputted the coordinates of two endpoints of
...
in a typical event.
The circles and triangles show the scattering point and the stopping point determined by the conventional method,
while the squares and rhombuses show the scattering point and the stopping point determined by the neural network.
...
It was found that this new method made the event selection and the track extraction more accurately and faster
